...
| Una versión / chunk publicado **"desaparece"** del registry (ej. la miniapp queda sin iOS pese a que el CI dijo `published … [ios]`) | Era un **lost update por concurrencia** (blob único en KV sin locking). **Resuelto (2026-09-02):** el registry es ahora una key por-miniapp con **compare-and-set** por-key → publishes concurrentes no se pisan (ver [SETUP](/docs/setup) §7.6). Si volviera a pasar: es un bug, reportalo. |
